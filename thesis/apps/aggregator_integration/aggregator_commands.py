...
def aggregate(path):
    ...
    subprocess.run(["java", "-jar", AGGREGATOR_SIMULATION, "-a", "-m", "SCTW", "-i", dump_trips_data, "-o", aggregation_path, "-O", "1000"])


def generate_energy_market(path):
    energy_market_name = os.path.join(path, ENERGY_MARKET_NAME)
    aggregation_path = os.path.join(path, AGGREGATION_NAME+".json")
    subprocess.run(["java", "-jar", AGGREGATOR_SIMULATION, "-g", "-e", "-i", aggregation_path, "-o", energy_market_name, "-C", "20"])
